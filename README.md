...
flowchart LR
    ...
    subgraph Phase1 [Phase 1: Completed]
        direction TB
        B1[✅ Gemini-based trip planner]
        B2[✅ Save Trip / Go on Trip]
        B3[✅ Local storage for user statistic]
        B4[✅ XP-based leveling & badges]
        B5[✅ Basic HP & Stamina mechanics]
        
        B1 --> B2 --> B3 --> B4 --> B5
    end

    %% Phase 2 – Current Development
    subgraph Phase2 [Phase 2: Ongoing]
        direction TB
        C1[Custom AI trip planner]
        C2[Quests, challenges & leaderboards]
        C3[Event-based tourism flows]
        C4[UI/UX improvements]
        C5[Authentication & cloud backup]
        
        C1 --> C2 --> C3 --> C4 --> C5
    end

    %% Phase 3 – Upcoming
    subgraph Phase3 [Phase 3: Future plan]
        direction TB
        D1[Blockchain-based rewards]
        D2[Partnerships with hotels & restaurant]
        D3[Multi-language suppor]
        D4[Social & group trip features]
        D5[Accessibility improvements]
    ...
    subgraph Phase4 [Phase 4: long term]
        direction TB
        E1[Global gamified tourism expansion]
        E2[AI-driven cultural discovery journey]
        E3[Sustainable tourism partnership]
        E4[Indian product marketplace]
...
graph LR
    %% Frontend Layer
    subgraph Frontend [Frontend & Mobile - Flutter]
        direction TB
        FlutterApp[Flutter Cross-platform App]
        UI[Gamified UI Components<br>Maps, Quests, Profile]
        State[State Management]
        AR[AR Features<br>Camera Integration]
    end
    ...
    subgraph Backend [Backend & Cloud]
        ...
        subgraph FirebaseCore [Firebase Core Services]
            Auth[Firebase Auth<br>Email, Social, Web3]
            Firestore[Cloud Firestore<br>User Data & Trips]
            Storage[Cloud Storage<br>Media & Assets]
            Functions[Cloud Functions<br>Serverless Logic]
        end
        
        subgraph APIServices [API & Microservices]
            TripAPI[Trip Management API]
            GamificationAPI[Gamification API]
            PartnerAPI[Partner Integration API]
            BlockchainAPI[Blockchain Bridge API]
        end
    end
    
    %% AI & Machine Learning Layer
    subgraph AI_Layer [AI Trip Planner]
        direction TB
        LLM[Custom LLM Model<br>Fine-tuned for Tourism]
        NLP[NLP Module<br>Multi-language]
        RecEngine[Recommendation Engine<br>Personalized Trips]
        Optimization[Trip Optimization<br>Route Planning]
    end
    
    %% Gamification Engine
    subgraph Gamification [Gamification Engine]
        direction TB
        XPSystem[XP & Leveling System]
        HPStamina[HP & Stamina Mechanics]
        BadgeSystem[Badge & Title System]
        QuestEngine[Quest & Challenge Engine]
        Leaderboard[Real-time Leaderboards]
    end
    
    %% Blockchain & Web3 Layer
    subgraph Blockchain [Blockchain & Web3]
        direction TB
        Wallet[Web3 Wallet Integration]
        TokenRewards[Token Reward System<br>Crypto & NFTs]
        SmartContracts[Smart Contracts]
        Marketplace[Digital Marketplace<br>Tourism Assets]
    end
    
    %% External Integrations
    subgraph External [External Services & APIs]
        direction TB
        Maps[Mapping APIs<br>Google Maps/Mapbox]
        Payment[Payment Gateways<br>Fiat & Crypto]
        Partners[Partner APIs<br>Hotels, Restaurants]
        TourismAPI[Tourism Board APIs<br>Local Data]
    end
    ...
    subgraph Github [GitHub]
        ...
        Docs[Documentation<br>Wiki & API Docs]
    end
    ...
    Functions --> TripAPI
    Functions --> GamificationAPI
    Functions --> PartnerAPI
    Functions --> BlockchainAPI
    ...
    GamificationAPI --> XPSystem
    GamificationAPI --> HPStamina
    GamificationAPI --> BadgeSystem
    GamificationAPI --> QuestEngine
    GamificationAPI --> Leaderboard
    
    BlockchainAPI --> Wallet
    BlockchainAPI --> TokenRewards
    BlockchainAPI --> SmartContracts
    BlockchainAPI --> Marketplace
    BlockchainAPI --> Payment
    
    XPSystem --> TokenRewards
    BadgeSystem --> TokenRewards
    QuestEngine --> TokenRewards
    ...
    classDef ai fill:#FFFDE7,stroke:#FBC02D,stroke-width:2px,color:#000000
    classDef gamification fill:#FFEBEE,stroke:#D32F2F,stroke-width:2px,color:#000000
    classDef blockchain fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#000000
    classDef external fill:#E0F2F1,stroke:#00796B,stroke-width:2px,color:#000000
    classDef github fill:#FCE4EC,stroke:#C2185B,stroke-width:2px,color:#000000

    class Frontend frontend
    class Backend backend
    class AI_Layer ai
    class Gamification gamification
...
|-------------|---------------|----------------|
| <img src="https://github.com/user-attachments/assets/8f2586f1-06c2-4558-bbe6-ca96992c8421" width="250"/> | <img src="https://github.com/user-attachments/assets/9b471327-26a8-4c35-9dd6-062634b25559" width="250"/> | <img src="https://github.com/user-attachments/assets/080d5319-8831-4cc9-acfd-08eabe761391" width="250"/> |

| My Trips | Go on Trip | Current Trip |
|-------------|---------------|----------------|
| <img src="https://github.com/user-attachments/assets/a263fe5c-b35b-48f8-a51b-a553dea34c7d" width="250"/> | <img src="https://github.com/user-attachments/assets/5f291b0a-e0e4-4bad-92f7-3641027a60c9" width="250"/> | <img src="https://github.com/user-attachments/assets/69841e41-6fb2-4b5e-abc3-da4fbe2d0d27" width="250"/> |
